...
| `empty` | string | `select`: string form of Kit `Select::empty` when the list has no rows. Kit accepts arbitrary `IntoElement`; custom empty widgets are not wrapped |
| `focus-ring` | bool | `select`: Kit `FocusableExt::focus_ring`. Omitted leaves Kit's true |
...
| `appearance` | bool | `hover-card`: Kit default popover chrome (Kit default true). Omitted leaves Kit's default. `select`: Kit `Select::appearance` (Kit default true) |
...
| `jump-button-label` | string | `message-scroller`: Kit `with_jump_button_label` |
...
| `jump-button-renderer` | object | `message-scroller`: Kit `with_jump_button_renderer` chrome (`variant`, `control-size`, `icon`, `tooltip`) |
