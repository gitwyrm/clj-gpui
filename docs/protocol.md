...
| `replace-generation` | number or string | `nav-stack`: same-id Kit `replace()` token. Changing it on the current `CljNavPage` entity creates a fresh page entity and calls `replace()` (forward is kept). Unchanged across rerenders is a no-op. Bound to that entity (not the catalog id) so a later navigation to another same-id history entry rebinds rather than replacing it |
