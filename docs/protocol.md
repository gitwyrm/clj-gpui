...
| `scroll-to-item` | string or number | `message-scroller`: Kit `scroll_to_item`. Row id, or a 0-based index if no row has that id. Omitted / JSON null leaves native scroll. Applied after child-list sync. `:scroll-to-end true` wins when both are set |
