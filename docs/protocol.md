...
| `fill` | hex, `{color}`, or `{stops, space, angle}` | `chart` `:bar`: Kit `BarChart::fill`. Point `fill` wins over node `fill` over point `color` over theme `chart_1`. `space` is `bar` (default; stop 0 = base, 1 = tip) or `chart` (remap stops through bar/chart pixel bounds). Optional `angle` degrees; omitted uses `BarAlignment::gradient_angle`. Area/radar series `fill` stays a hex string. `fill-gradient` replaces this when set |
