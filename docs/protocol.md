...
Protocol version is **5**. Clojure sends it on `:ready`. The host refuses a mismatch.
...
```json
{"op":"ready","protocol-version":5,"nrepl":7888,"app":"counter.app/app"}
```
...
`Cmd` (`Callback` / `CallbackBatch`) is host-internal and is not part of the JSON protocol version. Protocol v5 is unchanged.
...
| Field | Type | Used by |
|---|---|---|
| `type` | string | all (`window`, `label`, `button`, `vstack`, `hstack`, `spacer`, `checkbox`, `scroll`, `text-field`, `switch`, `toggle`, `radio-group`, `slider`, `progress`, `divider`, `spinner`, `tag`, `alert`, `skeleton`, `kbd`, `link`, `group-box`, `badge`, `tabs`, `select`, `icon`, `clipboard`, `breadcrumb`, `avatar`, `accordion`, `description-list`, `dialog`, `popover`, `dropdown-menu`, `context-menu`, `list`, `table`, `tree`) |
| `id` | string | optional stable identity, especially `text-field`, `slider`, `select`, `list`, `table`, `tree`, `dialog` |
...
| `children` | array of nodes | layouts, `scroll`, `group-box`, `badge`, `dialog`, `popover`, `context-menu` |
| `items` / `options` | array of `{id,label,text,disabled,content,on-click,span,items,cells,separator,width,checked,icon,expanded}` | `radio-group`, `select`, `tabs`, `breadcrumb`, `accordion`, `description-list` (`span` is description-list column span). Nested `items` are menu submenus / tree children. Table **columns** are `options`; table **rows** are `items` with `cells`. Do not reuse `columns` (u32) for table column defs |
...
| `on-change` | string callback id | `text-field` (string), `switch`/`toggle` (bool), `slider` (number), `select`/`radio-group`/`tabs`/`breadcrumb`/`accordion`/`list`/`table`/`tree`/`dropdown-menu`/`context-menu` (wire id; Clojure restores the original id). Accordion `:multiple` sends a JSON array in original item order |
...
| `on-confirm` | string callback id | `list` (click / Enter; original Clojure row id). Arrows only fire `on-change`; click/Enter fire `on-change` then `on-confirm` as one batch |
...
| `value` | JSON number, string, array, bool, or null | `slider`/`progress` (number), `select`/`radio-group`/`tabs`/`list`/`table`/`tree` (selected id or `null` to clear), `accordion` (id, `null`, or array of ids when `multiple`) |
...
| `open` | bool | `dialog`, `popover`: controlled open (`:open?` in Clojure). Omitted/false dialogs are not shown |
| `overlay-closable` | bool | `dialog`: click the dimmed overlay to dismiss (default true). Crate `confirm()`/`alert()` turn this off; the host restores the default unless this is `false` |
...
| `variant` | string | `button`, `tag`, `alert`, `tabs`, `group-box`, `toggle`, `dialog` (`confirm` / `alert`) |
| `compact` | bool | `button` |
...
| `title` | string | `window` (or any root): native window title (default `clj-gpui`). Also `alert` / `group-box` / `dialog` titles |
...
Functions never go on the wire. `gpui.runtime` replaces `fn?` values under `:on-click` / `:on-change` / `:on-submit` / `:on-double-click` / `:on-blur` / `:on-escape` / `:on-close` / `:on-copied` / `:on-ok` / `:on-cancel` / `:on-confirm` / `:on-open-change` with ids such as `"cb-2"`. Nested `:items` / `:options` / `:content` / `:trigger` are walked too. The registry is rebuilt on every export.
...
gpui-component 0.5.1 `Root::render` does not paint the dialog layer; the host calls `Root::render_dialog_layer` from `RootView`. Open/close still goes through `WindowExt` on the next frame so `RootView::render` does not re-enter `Root`. The dialog builder runs on every layer paint and reads a live spec cell (latest callback ids, title, body, children) so an unrelated Clojure rerender cannot leave a stale `cb-7` on an already-open dialog. Title and body update immediately while the dialog stays open. Overlay click dismisses the dialog by default even when `:variant` is `confirm` (crate `confirm()` turns that off; `:overlay-closable false` restores the crate lock). Crate order: OK → `:on-ok` then `:on-close`; Cancel / Escape / X / overlay → `:on-cancel` then `:on-close`. `:on-open-change false` is sent from `:on-close`. For one native action the host buffers those ids and sends **one** callback batch so `:on-ok` cannot render and rewire `:on-close`. After overlay/Escape dismiss the host does not re-open until Clojure’s tree drops `open`. Static overlay children use a full path element id. `popover` is in-tree; its trigger must be a button (`Selectable`). Menu item clicks send the original Clojure id; item `:on-click` then menu `:on-change` is one batch. List `:on-change` is selection and `:on-confirm` is activation; both restore the original Clojure id and, on click/Enter, run as one batch before the next tree.
