...
| `replace-generation` | number or string | `nav-stack`: same-id Kit `replace()` token. Changing it while the current page id stays the same creates a fresh page entity and calls `replace()` (forward is kept). Unchanged across rerenders is a no-op. Bound to the current top page so a later navigation cannot apply a stale bump |
