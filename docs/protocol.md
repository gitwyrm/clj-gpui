...
| `tooltip` | string | any node: GPUI Kit tooltip. `button` uses Kit `Button::tooltip` (not the generic wrapper) |
...
| `accessibility-label` | string | declarative `table`: Kit `Table::accessibility_label` (screen-reader name). A visible `table-caption` is not used as that name. `button` / `progress` / `progress-circle`: Kit `accessibility_label`. `input` / `textarea` / `editor`: Kit `aria_label`. Input also maps a present `id` to Kit `accessibility_id` |
| `href` | string | `link` |
...
| `icon` | string | `icon`, `spinner` (kebab `circle-check`); `button` / `alert` / `badge`; `avatar` placeholder icon (Kit default User); `select` / `combobox` trigger chevron; `input` / `number-input`: prefix glyph when `prefix` is omitted |
...
| `appearance` | bool | `hover-card`: Kit default popover chrome (Kit default true). Omitted leaves Kit's default. `select` / `combobox`: Kit `appearance` (Kit default true). `input` / `textarea` / `number-input` / `editor`: Kit field chrome (omit = Kit true). `kbd`: Kit `Kbd::appearance` (Kit default true) |
...
| `secondary` | bool | `skeleton`: Kit `secondary()`. Also accepted as `variant: secondary` |
