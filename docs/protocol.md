...
| `fill` | hex, `{color}`, or `{stops, space, angle}` | `chart` `:bar`: Kit `BarChart::fill`. Point `fill` wins over node `fill` over point `color` over theme `chart_1`. `stops` is exactly two `{color,at}` entries (`linear_gradient`); other lengths are dropped. `space` is `bar` (default; stop 0 = base/zero, 1 = tip; a negative value flips the omitted angle 180°) or `chart` (remap those two stops through bar/chart pixel bounds on the unflipped alignment axis). Optional `angle` degrees; omitted uses `BarAlignment::gradient_angle` (plus the bar-space sign flip). Area/radar series `fill` stays a hex string. `fill-gradient` replaces this when set |
