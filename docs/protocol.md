...
| `on-export` | string callback id | `data-table`: Kit `TableState::dump` as `{"headers":[…],"rows":[[…]]}`. Replay token is `export-generation`. Deferred so the callback cannot re-enter `export-tree` during `RootView::render`. Dump text is not option-id restored |
...
| `export-generation` | number or string | `data-table`: replay token for `TableState::dump`. Same shape as nav-stack `replace-generation`. A new token with `on-export` dumps the current native headers/rows |
