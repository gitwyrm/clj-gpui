...
| `trigger` | node | `popover`, `dropdown-menu` (usually a `button`); `hover-card` (any widget) |
...
| `placement` | string | `sheet`: `left` / `right` / `top` / `bottom` (default `right`). `hover-card`: Kit `Anchor` (`top-center` default; also `top-left` / `top-right` / `bottom-*` / `left` / `right`) |
