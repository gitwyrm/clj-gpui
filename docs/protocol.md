...
| `searchable` | bool | `select` / `combobox`: show a filter field; host uses `SearchableVec` so typing actually filters. Nested `options[].items` are Kit `SelectGroup` sections (`IndexPath` section+row). Combobox defaults true in `ui/combobox`. `list`: filter rows by label |
