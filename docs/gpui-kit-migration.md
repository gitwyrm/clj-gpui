...
| `Command`, `NativeMenu`, `StatusBar` | `ui/command`, `ui/native-menu`, `ui/status-bar` | Clojure owns the semantic menu/command tree; the host materializes Kit NativeMenu as a show-time snapshot and holds CommandState. A generic host Action (`slot` + item id) bridges Kit's required `Box<dyn Action>` without exposing GPUI Action types or capturing `cb-N`. StatusBar is independent (no Action). |
