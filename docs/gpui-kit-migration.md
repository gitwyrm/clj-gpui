...
| `NavStack` | `ui/nav-stack` | Clojure-owned `:stack` of page ids; host `NavStackState` + `push`/`pop`/`replace`/`clear`. `:transition` seconds (timing only). `:transition-style :slide` is opt-in; clipping is opt-in. Remaining: `forward`, `forward_views()`, custom `item` beyond `:slide`, dedicated `pop_to_root`. |
