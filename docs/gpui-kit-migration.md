...
| `NavStack` | `ui/nav-stack` | Clojure-owned `:stack` of page ids; host `NavStackState` + `push`/`pop`/`forward`/`pop_to_root`/`replace`/`clear`. Host-side forward branch of retained entities; `:on-forward-change` is `forward_views()` nearest-first. `:transition` seconds (timing only). `:transition-style :slide` is opt-in; clipping is opt-in. Remaining: custom `item` with complete `NavPage` context (`index`, `phase`, `operation`, eased `progress`). |
