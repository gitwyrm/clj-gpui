...
| `DataTable` extras | flags on `ui/data-table` | **Wrapped.** `:header-groups`, `:cell-selectable` / `:row-header`, pixel `:row-height` / named `:size`, `:export-generation` + `:on-export`. `SelectColumn` is not `:on-change`. Remaining Kit surface: stripe/bordered, sort/fixed/load-more, context menus, arbitrary `render_td`. |
