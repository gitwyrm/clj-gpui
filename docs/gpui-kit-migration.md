...
| `Message`, `Bubble`, `Attachment`, `Marker`, `MessageScroller` | `ui/message`, `ui/bubble`, `ui/attachment`, `ui/marker`, `ui/message-scroller` | First-class Kit nodes (not `{from, text}`). Host-held `MessageScrollerState`. `with_stack_style` / scroller `with_*_style` / Marker `ShimmerStyle` unwrapped. |
