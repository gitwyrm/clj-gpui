...
| `message_scroller::MessageScroller` | `ui/message-scroller` | ✅ | C | Host-held `MessageScrollerState` (tail follow on). Stable row `:id` for append/prepend without `reset`. Omitted `:scrollbar` / `:jump-button` keep Kit true. `:jump-button-transition` is seconds. Nested `:content-style` / `:list-style` / `:row-style` / `:jump-button-style` are Kit `with_*_style`. `:jump-button-renderer` is button chrome for `with_jump_button_renderer`. Row fingerprints ignore generated callback ids; append/prepend remeasures when a surviving row's render data changed. `scroll_to_item` / `scroll_to_end` are remaining (child-list sync cannot express programmatic navigation). Rows paint the static overlay subset plus this chat family (not list/data-table/editor) |
...
Chat `NavStack` is still a follow-up, as are `Command` / `NativeMenu` / `StatusBar` and remaining DataTable extras. `MessageScrollerState::scroll_to_item` / `scroll_to_end` are remaining chat APIs (child-list sync cannot jump to an existing row). `gpui-fps` paints on `:chrome :dev`. `gpui-shell` and `gpui-wry` are **intentionally unsupported**, not accidental gaps: Clojure is already the scripting layer, and WebView is a separate stack with no current reason to ship it inside clj-gpui.

## Category C — remaining

Grouped searchable `ui/select` (`SelectGroup` / `IndexPath`) is wrapped. Remaining C work is `NavStack`, `Command` / `NativeMenu` / `StatusBar`, Combobox `SelectGroup` sections, custom Select/Combobox row widgets (`Select::empty` as arbitrary `IntoElement`, `display_title` as `AnyElement`, custom row/section `render`), remaining DataTable public extras, and MessageScroller `scroll_to_item` / `scroll_to_end`. Full LSP for the code editor is out of scope; `ui/editor` is the highlighter widget. Kit 0.6 charts are wrapped on `ui/chart` without extra host limits (helpers such as `ui/horizontal-bar-chart` stay). `gpui-shell` and `gpui-wry` are the only intentional subsystem exceptions.
...
Kit `Message` / `Bubble` / `Attachment` / `Marker` are `RenderOnce` with no Entity. The wire is Kit primitives (`message-header`, `bubble`, `attachment-title`, …) so Ghost bubbles still go through `MessageContent::bubble` and reaction buttons through `BubbleReactions::action`. Named Clojure keys (`:avatar`, `:header`, `:footer`, `:reactions`, `:overlay`) expand to those children; they must not be confused with sheet `:footer`. Nested style maps cover `Message::with_stack_style`, `AttachmentTitle` / `Marker` `ShimmerStyle`, `Marker::separator_style`, and MessageScroller `with_*_style` / `with_jump_button_renderer`. Chat nodes use the same visual/layout style vocabulary as ordinary clj-gpui widgets. `MessageScroller` is the class-C host-state piece: one `MessageScrollerState` per `:id`, observed so the jump-to-latest button can appear. Collection diff is leave / remeasure / append / prepend / reset. Row fingerprints ignore generated callback ids; append/prepend also remeasures when a surviving row changed. Stable row `:id` is required for history prepend without `reset` (which re-enables tail follow). Index-only keys make prepend look like append. Scroller rows cannot re-enter `RootView`; they paint the overlay static subset plus this chat family. `scroll_to_item` / `scroll_to_end` are remaining.
