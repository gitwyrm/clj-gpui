...
| `base::NavStack` | `ui/nav-stack`, `ui/nav-page` | ⚠️ | C | Host-held `NavStackState`. Clojure `:stack` / `:value` is page ids root-first; children are `ui/nav-page` templates, not the live trail. Omitted stack is the first page id; only `[]` clears. An explicit trail with an unknown page id is rejected (native stack unchanged). Host diffs and calls Kit `push`/`pop`/`replace`/`clear`. Each stack entry is a distinct `CljNavPage` entity from the catalog template (repeated ids are two history entries). Live cells `notify()` so regenerated callback ids paint. `:transition` is seconds (wire `duration`) → Kit `transition()` only; omitted / ≤0 is immediate. `:motion :immediate` skips animation. `:transition-style :slide` opts into the showcase `item` renderer; omitted keeps Kit's default `NavPage` renderer. `:overflow :hidden` / `:overflow-hidden true` clip; omitted does not. Pages paint the overlay static subset plus chat (not list/data-table/editor). Remaining: Kit `forward`, `forward_views()`, `pop_to_root` as a dedicated op (multi-pop rebuilds immediately), and a custom `item` renderer beyond `:slide` |
