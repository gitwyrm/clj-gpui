...
| `message::Message` / `MessageGroup` / `MessageAvatar` / `MessageHeader` / `MessageContent` / `MessageFooter` | `ui/message`, `ui/message-group`, plus slot constructors | ✅ | C | Named slots on the wire. `:alignment` `:start` / `:end`. A `ui/bubble` inside `message-content` is Kit `.bubble` so Ghost still strips header/footer inset. Named `:footer` is a child, not sheet `footer`. `with_stack_style` is not wrapped |
| `bubble::Bubble` / `BubbleContent` / `BubbleGroup` / `BubbleReactions` | `ui/bubble`, `ui/bubble-content`, `ui/bubble-group`, `ui/bubble-reactions` | ✅ | C | Variants `filled` (default) / `secondary` / `muted` / `tinted` / `outline` / `ghost` / `destructive`. Reaction `:side` `top` / `bottom`. `ui/button` children of reactions use Kit `.action` (pill geometry) |
| `attachment::*` | `ui/attachment`, `ui/attachment-media`, `ui/attachment-content`, `ui/attachment-title`, `ui/attachment-description`, `ui/attachment-actions`, `ui/attachment-group` | ✅ | C | `:status` pending/uploading/processing/failed/complete. Whole-card click needs `:id` and `:on-click`. Title/description inherit status shimmer. `AttachmentGroup` needs `:id` |
| `marker::*` | `ui/marker`, `ui/marker-icon`, `ui/marker-content` | ✅ | C | `:variant` plain/separator/border. `:loading` plus `:loading-style` spinner/shimmer. `:role :status` needs `:id`. Custom `ShimmerStyle` / separator `StyleRefinement` are not wrapped |
| `message_scroller::MessageScroller` | `ui/message-scroller` | ✅ | C | Host-held `MessageScrollerState` (tail follow on). Stable row `:id` for append/prepend without `reset`. Omitted `:scrollbar` / `:jump-button` keep Kit true. `:jump-button-transition` is seconds. Custom jump-button renderer and `with_*_style` are not wrapped. Rows paint the static overlay subset plus this chat family (not list/data-table/editor) |
...
Chat `NavStack` is still a follow-up, as are `Command` / `NativeMenu` / `StatusBar` and remaining DataTable extras. `gpui-fps` paints on `:chrome :dev`. `gpui-shell` and `gpui-wry` are **intentionally unsupported**, not accidental gaps: Clojure is already the scripting layer, and WebView is a separate stack with no current reason to ship it inside clj-gpui.

## Category C — remaining

Grouped searchable `ui/select` (`SelectGroup` / `IndexPath`) is wrapped. Remaining C work is `NavStack`, `Command` / `NativeMenu` / `StatusBar`, Combobox `SelectGroup` sections, custom Select/Combobox row widgets (`Select::empty` as arbitrary `IntoElement`, `display_title` as `AnyElement`, custom row/section `render`), and remaining DataTable public extras. Full LSP for the code editor is out of scope; `ui/editor` is the highlighter widget. Kit 0.6 charts are wrapped on `ui/chart` without extra host limits (helpers such as `ui/horizontal-bar-chart` stay). `gpui-shell` and `gpui-wry` are intentionally unsupported.
...
Kit `Message` / `Bubble` / `Attachment` / `Marker` are `RenderOnce` with no Entity. The wire is Kit primitives (`message-header`, `bubble`, `attachment-title`, …) so Ghost bubbles still go through `MessageContent::bubble` and reaction buttons through `BubbleReactions::action`. Named Clojure keys (`:avatar`, `:header`, `:footer`, `:reactions`) expand to those children; they must not be confused with sheet `:footer`. `MessageScroller` is the class-C host-state piece: one `MessageScrollerState` per `:id`, observed so the jump-to-latest button can appear. Collection diff is leave / remeasure / append / prepend / reset. Stable row `:id` is required for history prepend without `reset` (which re-enables tail follow). Index-only keys make prepend look like append. Scroller rows cannot re-enter `RootView`; they paint the overlay static subset plus this chat family. `with_stack_style`, scroller `with_*_style`, `with_jump_button_renderer`, and Marker `ShimmerStyle` are remaining.
