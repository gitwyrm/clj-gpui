...
| `table::Table` (declarative) | `ui/table` | ✅ | C | Not virtualized. Columns `{label, width, align, span}` → `options`; rows are cell vectors or `{id, cells}`; `:footer` is last item `variant: "footer"`; `:caption` is node `text` |
| `combobox::Combobox` | `ui/combobox` | ✅ | C | Host `ComboboxState<SearchableVec>`. Search on by default. `:multiple true` value is a vector; `:on-confirm` on popover close |
| `rating::Rating` | `ui/rating` | ✅ | C | Integer `0..=:max` (default 5). `:on-change` is the new integer. Optional `:color` hex |
...
Declarative `ui/table` is not this family: Kit `Table` / `TableHeader` / `TableBody` / `TableFooter` / `TableCaption` are `RenderOnce` with no slot, no virtualization, and no selection. The host applies Clojure layout/style keys only (`apply_style`, not `viewport_sized`).
...
| `combobox` | `:on-change` / `:on-confirm` | original Clojure option id, or a vector of ids when `:multiple true`. `:on-confirm` fires when the menu closes |
