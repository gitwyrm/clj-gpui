...
| `base::NavStack` | `ui/nav-stack`, `ui/nav-page` | ⚠️ | C | Host-held `NavStackState`. Clojure `:stack` / `:value` is page ids root-first; children are `ui/nav-page` templates, not the live trail. Omitted stack is the first page id; `[]` clears. Host diffs and calls Kit `push` / `pop` / `replace` / `clear`. `:transition` is seconds (wire `duration`); omitted / ≤0 is immediate. `:motion :immediate` skips animation. Default slide item renderer when a transition is set. Pages paint the overlay static subset plus chat (not list/data-table/editor). Remaining: Kit `forward` (re-adding a popped id is a new push and discards the forward branch), custom `item` renderer, `pop_to_root` as a dedicated op (multi-pop rebuilds immediately) |
