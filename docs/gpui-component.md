...
| `input` code editor | `ui/editor` | ✅ | C | `InputState::code_editor` highlighter. `:language` (default `text`). **No LSP** |
...
| `dock::*` / Tiles | `ui/dock` | ✅ | C | Tabs per side from item `:side`. Panel bodies: static overlay subset + markdown/chart (not list/table/editor). No layout persistence |
...
Clojure stays the semantic owner. Rust holds widget `Entity` state only where GPUI requires it. Number-input reuses the text-field `InputState` slot with a `NumberInput` wrapper; step events parse the current text, apply `:step` / `:min` / `:max`, and `set_value` (which emits `:on-change` as a JSON number). OTP `:on-change` is crate-complete-only; incomplete typing is not overwritten while focused. Color is hex. Date is ISO; `set_date` is skipped when the value is unchanged so an open picker is not closed every frame. Editor is highlighter-only (`set_highlighter` on language change). Undo/redo groups and fast typing emit several crate `Change` events; the host defers one `:on-change` with the latest string and will not send another until the next tree assigns a fresh `cb-N` (export-tree is monotonic, so a second send of the same id is `unknown callback`). Dock `CljPanel::panel_name` is always `"clj-gpui-panel"`; panel paint does not re-enter `RootView` (static overlay painter + markdown/chart). Settings fields capture current values each `RootView` frame (`Settings` is `RenderOnce`).

## Path to near-complete coverage

Clojure stays the semantic owner. Rust holds widget `Entity` state only where GPUI requires it. The slot map now covers text-field, slider, select, list, table, tree, OTP, color, date, editor, virtual-list, dock, and resizable. Overlay sync covers dialog, sheet, and notification; popover/menus are in-tree. Remaining C work is slider range/log, select sections, DropdownButton, and AvatarGroup — not a new architecture.

## Callback payloads (protocol v6)
...
| `dialog` / `popover` / `sheet` | `:on-open-change` | boolean |
