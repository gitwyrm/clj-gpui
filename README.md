...
* **`preview-png` is an OS window shot**, not GPU readback. Linux/Windows spawn `clj-gpui --capture-preview --pid <host-pid>` and [xcap](https://crates.io/crates/xcap). macOS captures in-process with ScreenCaptureKit after keeping GPUI's display link running while occluded ([zed#63217](https://github.com/zed-industries/zed/issues/63217)). Wayland portals and missing macOS Screen Recording permission return `nil`. Xvfb/X11 is the deterministic Linux CI path.
