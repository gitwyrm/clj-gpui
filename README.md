...
* **`preview-png` is an OS window shot**, not GPU readback. Linux/Windows spawn `clj-gpui --capture-preview --pid <host-pid>` and [xcap](https://crates.io/crates/xcap). macOS captures in-process with ScreenCaptureKit. GPUI 0.2.2 otherwise stops painting while the window is occluded ([zed#63217](https://github.com/zed-industries/zed/issues/63217)); the host overrides `GPUIWindow`'s `occlusionState` so the display link keeps presenting under Evalight. `inactive_frame_interval` ([zed#62628](https://github.com/zed-industries/zed/pull/62628)) is not in 0.2.2 and does not control occlusion. Wayland portals and missing macOS Screen Recording permission return `nil`. Xvfb/X11 is the deterministic Linux CI path.
